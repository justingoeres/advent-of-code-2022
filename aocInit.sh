...
function main() {
  ...
  if [[ -z ${no_templates-} ]]; then
    # DayXX.template.ts
    templateFile2=DayXX.template.ts
    outputFile2=${templateFile2/XX.template.ts/"$day".ts} # e.g. DayXXServiceTemplate.java -> Day01ServiceTemplate.java
    instantiateTemplate "$templateDir/$templateFile2" "$srcDir/$outputFile2"

    # DayXX.test.template.ts
    templateFile3=dayXX.test.template.ts
    outputFile3=${templateFile3/XX\.test\.template.ts/"$day".test.ts} # e.g. DayXXTestTemplate.java -> Day01TestTemplate.java
